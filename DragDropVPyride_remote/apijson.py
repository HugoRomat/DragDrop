...
class PostPose(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        print pose
        self.finish()
        pyride_remote.takecontrol()
        pyride_remote.issue_command( 31, pose)

class Experience(tornado.web.RequestHandler):
    def set_default_headers(self):
        self.set_header('Access-Control-Allow-Origin', '*')
        self.set_header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
        self.set_header('Access-Control-Max-Age', 1000)
    ...
    def post(self):
        #my_data = self.request.arguments['test_string']
        num = 31
        pose = self.get_argument('experiment')
        print pose
        if int(pose) == 1:
            num = 41
            print "1"
        ...
        self.finish()
        pyride_remote.takecontrol()
        pyride_remote.issue_command( num, pose)
...
class AjaxHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        self.finish()
        pyride_remote.takecontrol()
        pyride_remote.issue_command( 30, couleur1+"/"+couleur2+"/"+couleur3+"/"+couleur4)
        #
...
if __name__ == "__main__":
    ...
    pyride_remote.connect('127.0.0.1', '1234')
